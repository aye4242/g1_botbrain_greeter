...
> 当前 Humble MPPI 不读取 `ax_max/ax_min/ay_max/ay_min/az_max/vy_min`。加减速限制应由机器人底层或启用后的 velocity smoother 负责，不能在本 YAML 中加入无效参数后假定已经生效。
...
| 速度太慢 | `vx_max` | 提高到 0.5 |
| 启动/停止太猛，机器人不稳 | 底层速度平滑、velocity smoother | 不要添加 Humble MPPI 不读取的加速度参数 |

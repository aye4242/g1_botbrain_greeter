...
场景选择器返回 `0` 后 Navigation 才会启动。启动后将 Foxglove Fixed Frame 设为 `map`，并重新执行 **2.3 使用成品地图做 Open3D ICP 验收** 中的 Z/TF、时间戳和连续 accepted 检查。若修订地图的验收不通过，立即停止 Navigation；只有自动全局初始化持续失败时才使用 `/initialpose` 应急回退。
...
若 `/cloud_registered_1`、`/scan`、`/path_1` 同时在 Foxglove 标红，执行：

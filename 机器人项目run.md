...
```bash
project_dir=/home/aitech/Workspace/botbrain_project
...
`Publisher count` 必须大于 `0`。若为 `0` 或 RViz 显示 `No transform`，依次检查 G1 IP、工作站与 G1 的网络连通性、机器人端 `docker compose ps zenoh` 是否为 `running`，然后重启上述 daemon 并再次检查。
...
- Fixed Frame 必须为 `camera_init`（预设默认值）。只看 `/cloud_registered_1`、`/accumulated_grid`、`/path_1` 和 `/Odometry_loc`；判断漂移时不要打开 `/cloud_registered_body_1`。
- `/cloud_registered_1` 应随环境累积，墙体不能跟随机器人旋转；`/accumulated_grid` 应逐渐形成连续的 2D 栅格。
...
- Fixed Frame 必须为 `map`（预设默认值）；预设显示 `/map`、`/pcd_map`、`/scan`、`/g1_robot/plan`。
...
```bash
cd /data/unitree/botbrain_ws
scene=floor1  # 本次建图的场景名；11 层分别使用各自统一名称
...
/**:
  ros__parameters:
    map_file_path: "/botbrain_ws/src/g1_pkg/maps/floor1_scans.pcd"  # floor1 换成 $scene 的实际值
...
# 后台启动基础服务和 Zenoh；RViz2 在 workstation 直连 Zenoh。
# 建图时 localization/navigation 必须停止，避免定位负载和旧地图发布者干扰建图链路。
docker compose stop localization navigation
docker compose up -d zenoh bringup state_machine

# 普通场景用 default；两侧有开门房间的长走廊改为 corridor。
FAST_LIO_MAPPING_PROFILE=default docker compose up -d --force-recreate fast_lio

# 机器人必须保持静止，直到 IMU 初始化完成。
# 看到 IMU Initial Done 和连续 timing ok=true 后按 Ctrl-C，后台容器不会停止。
docker compose logs -f fast_lio | \
  grep -E "IMU Initial|FAST-LIO input|FAST_LIO_PCD|FAST_LIO_TIMING|FAST_LIO_GUARD|FAST_LIO_RANGE"

# 宿主机不要求安装 ROS；在 FAST-LIO 容器内检查栅格话题。
...
- 默认模式启动日志包含 `imu=/livox/imu flip_yz=true imu_q=400 lidar_q=20 max_imu_gap=0.0300s max_range=0.0m guard=true`；走廊模式应显示 `max_range=20.0m`。
...
- 判断地图稳定性只观察 `/cloud_registered_1`；建图预设将其保留最近 `15` 秒，长期地图轮廓以 `/accumulated_grid` 和 `/path_1` 为准，避免长走廊运行时 RViz2 持续累积点云导致卡顿。
...
source /opt/ros/humble/setup.bash
source /botbrain_ws/install/setup.bash
scene=floor1  # 必须与步骤 0 的 scene 完全一致
...
最后必须恢复导航安全模式。下面命令只修改唯一的 `pcd_save_en` 键；完成后同时验证 source 和 install。定位/导航阶段保持保存关闭，既避免覆盖已校正 PCD，也不会启动建图用的 `grid_accumulator`：

```bash
cd /data/unitree/botbrain_ws
cfg=botbrain_ws/src/fast_lio/config/mid360.yaml
sed -i -E 's/^([[:space:]]*pcd_save_en:)[[:space:]]*.*/\1 false  # 导航安全默认；仅正式建图时临时改为 true/' "$cfg"
...
for cfg in \
  ...
  grep -Eq '^[[:space:]]*pcd_save_en:[[:space:]]*false([[:space:]]|$)' "$cfg" || {
    echo "ERROR: $cfg 尚未恢复 pcd_save_en=false" >&2; exit 1;
  }
done
```
...
- 暂时关闭 `/cloud_registered_body_1`、`/cloud_effected_1`、`/scan2map` 和 `/submap`，避免不同用途的点云叠在一起。

`camera_init` 只用于检查 FAST-LIO 自身建图是否稳定。导航时若仍以 `camera_init` 为 Fixed Frame，RViz2 的 `2D Pose Estimate` 会把 `/initialpose` 标成 `camera_init`，定位节点将拒绝它；而启动时一次性发布的 `/pcd_map` 也可能因旧时间戳显示在错误高度，看起来像实时点云整体位于地图下方。
...
robot_ip=192.168.100.30         # 改为 G1 实际 IP
maps=botbrain_ws/src/g1_pkg/maps
...
# Workstation RViz2：标准可视化并通过 /g1_robot/goal_pose 发送导航目标
project_dir=/home/aitech/Workspace/botbrain_project
g1_ip=192.168.100.30
source /opt/ros/humble/setup.bash
export RMW_IMPLEMENTATION=rmw_zenoh_cpp
...
local costmap 的配置是 `g1_robot/odom` 坐标系下 `8m x 8m` 的 `rolling_window=true`，正常时机器人应在窗口中心。RViz2 中需要手动 Add 后启用 `/g1_robot/local_costmap/costmap` 图层，不能在图层未启用时判断未居中。先打开该话题，再用同一 costmap 时间戳的 TF 做数值验证：

...
> **可视化标准：RViz2。** 建图、定位和导航均在 workstation 本地运行 RViz2，并通过 Zenoh 直连机器人；不再启动或使用 Foxglove，以避免网络不稳定时 WebSocket 可视化卡顿。机器人端必须运行 `zenoh` 服务（TCP `7448`），工作站必须安装 `ros-humble-rmw-zenoh-cpp` 和 `ros-humble-rviz2`。完整界面操作见同目录的 `rviz-guide.md`。
...
以下命令在 **workstation** 执行，IP 改为机器人当前 Wi-Fi IP。脚本会完成 ROS/Zenoh 环境设置并强制加载项目预设，不需要手动 source、export、Add 话题或执行 `rviz2 -d`。

```bash
cd ~/Workspace/g1_botbrain_greeter

# 今天建图：Fixed Frame=camera_init，两类点云无需定位即可显示
bash tools/host_side/mapping_rviz2.sh 192.168.37.204

# 定位/导航：场景选择器打印 RVIZ POINT CLOUD READY 后即可打开，Fixed Frame=map
# bash tools/host_side/g1_nav_loc_rviz2.sh 192.168.37.204
```

若机器人 IP 变成 `192.168.100.3`，只替换脚本最后一个参数。脚本连接失败时再检查 G1 IP、工作站网络及机器人端 `docker compose ps zenoh`；Zenoh 端点必须是 `tcp/<IP>:7448`，不能写成 `tcp://<IP>:7448`。

- Fixed Frame 必须为 `camera_init`（预设默认值）。预设已包含 `/cloud_registered_1`、`/cloud_registered_body_1`、`/accumulated_grid`、`/path_1`、`/Odometry_loc` 和 TF，不需要手动 Add；启动检查以 `/cloud_registered_1` 为硬性门槛，`/cloud_registered_body_1` 若因 Zenoh 发现延迟暂时 Warm，等待 RViz 自动发现即可，判断漂移时以 `/cloud_registered_1` 为准。
- `/cloud_registered_1` 是每帧世界坐标点云，不是机器人端整图话题。建图预设用 `world scan (live)` 显示当前帧，并用 `world history (5 min)` 在工作站保留最近 300 秒；墙体不能跟随机器人旋转。`/accumulated_grid` 是全程累计的 2D 栅格。
- 现场是长走廊时建图使用 `FAST_LIO_MAPPING_PROFILE=corridor`。为了不让移动 Wi-Fi 上的大栅格持续拖慢 Jetson，`/accumulated_grid` 改为 0.5 Hz 发布、每 3 帧云处理一次；FAST-LIO 主点云仍保持 10 Hz。若日志出现 `output latched unhealthy`，这是已判定不安全的估计状态，必须重建 FAST-LIO，不能靠继续等待恢复。

- Fixed Frame 必须为 `map`（预设默认值）；预设自动显示 `/map`、高亮蓝色 `/pcd_map`、紫色 `/scan2map` 定位候选、绿色 `/cloud_registered_1`、橙色 `/cloud_registered_body_1`、`/scan`、`/g1_robot/plan` 和 TF，并预置默认关闭的 global/local costmap，不需要手动 Add。
- 定位尚未 ready 时，蓝色 `/pcd_map` 和持续刷新的紫色 `/scan2map` 必须已经可见。紫色层先显示未验证实时扫描，算法得到候选后再更新到候选位置；它不代表导航已经放行。绿色/橙色实时点云因为缺少经验证的 `map` TF 可以暂时 Warm，定位完成后自动出现。
- 如果连蓝色 `/pcd_map` 都为空，不要盲发初始位姿；先等机器人端场景选择器打印 `RVIZ POINT CLOUD READY`，并确认 `docker compose ps localization` 显示 `Up`。
- 当前预设不包含 RobotModel。更新预设后必须关闭旧 RViz2 再运行启动脚本；仍看到 `RobotModel (G1)` 说明看的是旧窗口。
- 只在 `Localization ready` 且 Nav2 lifecycle 就绪后，用工具栏 `2D Goal Pose` 在地图上点击拖拽，向 `/g1_robot/goal_pose` 发布导航目标。
- 自动全局定位持续失败时，才用工具栏 `2D Pose Estimate` 发布 `/initialpose`；发送时 Fixed Frame 必须仍是 `map`，否则定位节点会拒绝该消息。
...
`g1_robot_foxglove` orphan 警告不影响 RViz2，可保留兼容，不需要为了使用 RViz2 执行 `--remove-orphans`。

> 本节是建图、定位、导航和多楼层切换的详细手册。现场只需要标准命令时，使用同目录下的 `建图导航指令.md`。
>
> 日常定位和切图的唯一标准入口是 `tools/nav/select_map_scene.sh`。不要用 `export MAP_SCENE=...`、`docker compose restart localization` 或只调用 `map_server/load_map` 代替该脚本；`MAP_SCENE` 是脚本创建 localization 容器时使用的内部环境键，未显式选择场景时 Compose 默认使用 `ug`，但已经运行的容器不会因为宿主机重新 `export` 而切图。
>
> `localization` 没有省略：它是 `navigation` profile 下的 Compose 服务，由场景选择器内部按目标场景执行 `docker compose ... up -d localization`。最终 `fast_lio + localization + navigation` 会同时运行，但必须先完成 localization 验证，才能启动 Navigation。定位/导航 RViz 的 Fixed Frame 始终是 `map`；`camera_init` 只用于建图 RViz。
>
> 本节命令统一使用 `scene` 变量。每次执行前只修改一次，例如 `scene=ug`、`scene=aitech`、`scene=floor4`，后续不要再手工混写其他楼层名。

阅读顺序：新建地图看“一 + 二”，修改 2D 地图看“三”，多楼层切换看“四”，日常定位导航和故障检查看“五”。
...
日常重复建图不需要重新编译，也不再手工修改 `mid360.yaml`。只有代码或 launch 更新后执行：
...
# 服务立即在后台运行；先看容器状态，再持续看 FAST-LIO 完整日志
docker compose ps zenoh bringup state_machine fast_lio
docker compose logs -f fast_lio
```

`docker compose up -d` 创建服务后立即返回，不再等待自动就绪检查；`logs -f` 中按 `Ctrl+C` 只退出日志跟随，容器仍在后台运行。上述环境变量会临时启用 PCD 保存、2D 累计栅格和本场景 PCD 路径；`mid360.yaml` 始终保持导航安全默认 `pcd_save_en=false`，建完后无需改配置或重新编译。

机器人保持静止，直到日志出现 `IMU Initial Done` 和连续 `[FAST_LIO_TIMING] ok=true`，并在建图 RViz2 中看到世界点云、Body 点云与 `/accumulated_grid`。服务形式不会自动备份同名地图：优先使用新场景名；确认重建旧场景时，启动前先备份 `<scene>_scans.pcd/.pgm/.yaml`。

建图期间不得另行启动 `localization` 和 `navigation`。`1.247 m` 是站立时传感器到地面的高度，不是 LiDAR 到内置 IMU 的外参；禁止把它写入 `extrinsic_T`，也禁止在 `imu_flip_yz=true` 时另启 `imu_flip.py`。

原 `start_mapping_scene.sh` 不删除，作为需要自动备份和等待 `MAPPING READY` 时的可选入口；现场默认不使用它：
...
- 默认模式启动日志包含 `imu=/livox/imu flip_yz=true imu_q=400 lidar_q=20 max_imu_gap=0.0600s max_range=0.0m guard=true`；走廊模式应显示 `max_range=20.0m`。
- 启动日志中的 profile 必须与所选模式一致；走廊模式还应显示 `max_range=20.0m`。
- 启动日志包含 `[FAST_LIO_PCD] enabled=true path=<目标 PCD> interval=-1 laser_map=false`；这里显示的是 `install/` 中实际生效的配置。
- 看到 `IMU Initial Done` 后才能移动。
- `[FAST_LIO_TIMING] ok=true` 应稳定出现；通常 `imu_count>=5`、`max_gap<=0.06s`（由 `common.max_imu_gap` 控制）、扫描末端 IMU 差不超过 `0.03s`。
...
### 步骤 2：用 RViz2 小范围验收后正式建图

RViz2 设置（先按本节开头“RViz2 工作站连接与使用”启动建图预设）：

- Fixed Frame：`camera_init`。
- Display/Follow Frame：`camera_init`。
- 判断地图稳定性只观察 `/cloud_registered_1`。建图预设同时打开当前帧和最近 `300` 秒历史：当前帧保证刚启动就能看到点云，5 分钟历史用于检查已走区域；更长时间的完整 2D 轮廓仍以 `/accumulated_grid` 和 `/path_1` 为准，最终完整 3D 数据以保存的 `${scene}_scans.pcd` 为准。
- `/cloud_registered_body_1` 已作为橙色实时扫描默认打开；它随机器人运动是正常定义，不能用它判断世界地图是否漂移。
- 建图预设只加载建图所需图层，不需要手动 Add。若导航预设处于 Fixed Frame=`map` 且定位未 ready，点云可能因缺少 map TF 而全部为空；关闭该窗口并重新运行建图脚本，确认 Fixed Frame=`camera_init`。

### 步骤 3：先保存 2D 栅格，再主动保存 PCD，最后停止 FAST-LIO
...
marker="$maps/.${scene}_mapping_started"
test -f "$marker"
test -s "$maps/${scene}_scans.pcd"
test "$maps/${scene}_scans.pcd" -nt "$marker"
head -n 11 "$maps/${scene}_scans.pcd" | \
  grep -E "^(FIELDS|WIDTH|HEIGHT|POINTS|DATA)"

# floor1 展开后的等价检查，用于现场逐字核对：
...
# 重新按本节步骤 1 设置 FAST_LIO_MAPPING_* 环境并重建 fast_lio 服务
```
...
scene=floor1
maps=/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps
ls -lh "$maps/${scene}_scans.pcd" "$maps/${scene}.pgm" "$maps/${scene}.yaml"
head -n 11 "$maps/${scene}_scans.pcd" | \
  grep -E "^(FIELDS|WIDTH|HEIGHT|POINTS|DATA)"
...
# raw 是不可覆盖的原始基准。若已存在，先确认当前成品是否已经校正，禁止继续执行。
test ! -e "$maps/${scene}_scans_raw.pcd" || {
  echo "ERROR: $maps/${scene}_scans_raw.pcd 已存在，拒绝覆盖原始 PCD" >&2
  exit 1
}
cp -a "$maps/${scene}_scans.pcd" "$maps/${scene}_scans_raw.pcd"

docker compose run --rm -e SCENE="$scene" builder_base bash -lc '
  source /opt/ros/humble/setup.bash
  source /botbrain_ws/install/setup.bash
  python3 /botbrain_ws/install/g1_pkg/lib/g1_pkg/shift_pcd_z.py \
    "/botbrain_ws/src/g1_pkg/maps/${SCENE}_scans.pcd" 1.247
'
```

脚本输出的 `z 1pct/5pct/med` 只用于确认每个分位数都准确增加了 `1.247m`，不能单独用来识别地面。FAST-LIO 特征 PCD 中地面点占比可能低于 5%，动态物体或地下噪声也会改变分位数，因此它们不一定落在 `0m`。最终地面高度和地图质量必须结合 3D 可视化、初始位姿高度以及 Open3D ICP 判断。若平移量或可视化明显不符，恢复 `${scene}_scans_raw.pcd` 后排查，不要再次叠加平移。

进入定位/导航时使用 `select_map_scene.sh --restart-fast-lio`。脚本会显式以 `FAST_LIO_MAPPING_MODE=false`、`FAST_LIO_MAPPING_SAVE=false` 和 `FAST_LIO_MAPPING_PROFILE=default` 重建 FAST-LIO，不会覆盖已经校正的成品 PCD；若误用不带 restart 的快捷命令，场景选择器也会识别仍在运行的建图容器并拒绝启动导航。
...
- YAML 的 `image` 应指向 `${scene}.pgm`，`resolution` 应为 `0.05`，`origin` 必须保留 map_saver 输出值。
...
- 在 RViz2/Open3D 地图显示中检查墙体是否重影、倾斜或大面积缺失。

### 2.3 使用成品地图做 Open3D ICP 验收

定位服务只需要一个场景名，并自动加载同目录中的 `<scene>_scans.pcd` 和 `<scene>.yaml`；YAML 再加载 `<scene>.pgm`。`select_map_scene.sh --restart-fast-lio` 会明确关闭建图保存模式。标准流程不再手改 `mid360.yaml`，也不创建或更新 `scans.pcd`、`accumulated.yaml` 两个符号链接。
...
定位/导航阶段的 RViz2 设置与建图阶段不同：

- Fixed Frame：`map`。
- 使用 `configs/g1_nav_loc_rviz2.rviz`；只用 `/pcd_map`、`/cloud_registered_1` 和 `/map` 检查对齐，点云 Decay 设为 `0`。
- 定位已 ready 并开始做最终 ICP 验收后，可暂时关闭 `/cloud_registered_body_1`、`/cloud_effected_1`、`/scan2map` 和 `/submap`，避免不同用途的点云叠在一起；定位前保留紫色 `/scan2map` 用于观察候选。

`camera_init` 只用于检查 FAST-LIO 自身建图是否稳定。导航时若仍以 `camera_init` 为 Fixed Frame，RViz2 的 `2D Pose Estimate` 会把 `/initialpose` 标成 `camera_init`，定位节点将拒绝它。定位节点现在每 5 秒补发一次 `/pcd_map`，让晚打开的 RViz/Zenoh 客户端也能收到静态 PCD；该 PCD 的 frame 始终是 `map`。

然后通过标准场景选择器重新创建 FAST-LIO 和定位服务。该步骤只做定位验收，暂不启动 Navigation：
...
场景选择器会检查三地图文件、清除旧 localization publisher、等待 FAST-LIO 连续时序健康，并实时显示当前 localization 容器本次启动后的匹配决策。地图和 PCD 发布者稳定后会先打印 `RVIZ POINT CLOUD READY`，此时即可打开导航 RViz2 查看蓝色成品 PCD，不必等选择器最终返回。它再检查 scan/Unitree twist/TF，要求连续 3 次 `accepted=true`、`fitness>0.50`、`rmse<=0.30`，然后复查 preflight 和关键话题 publisher。`--ready-timeout 300` 是上述就绪检查共用的总时限；任何非零返回都表示当前场景不能启动 Navigation。地图已经成功加载后若只是定位超时，localization 会保留供 RViz 诊断，但 Navigation 仍保持停止。
...
```text
Global initialization: enabled=true ... confirmations=3 scan_window=10
...
机器人启动位置不在建图起点并不是立即手工发送 `/initialpose` 的理由。先保持机器人静止，让全局候选完成 `3/3` 确认。只有日志长时间反复出现 `did not produce a valid FPFH/RANSAC candidate`、`rejecting global candidate` 或始终无法累积到 `3/3` 时，才通过 RViz2 的 `2D Pose Estimate` 给出实际位置作为失败回退。发送后必须看到：

发送前必须先在 RViz2 看见高亮蓝色 `/pcd_map` 和持续刷新的紫色 `/scan2map`，并以蓝色 PCD 和二维 `/map` 为参照点击机器人真实位置。紫色层在无候选时是未验证实时扫描，有候选后会移动到候选位置；候选接近正确位置时应贴合蓝色 PCD。绿色/橙色实时点云只有在定位验证成功、`map` TF 建立后才会从 Warm 自动变为可见。
...
docker exec -it g1_robot_localization bash -lc '
  source /opt/ros/humble/setup.bash
  source /botbrain_ws/install/setup.bash
  ros2 run bot_navigation navigation_preflight.py \
    --ros-args -p timeout_sec:=300.0
...
若日志显示 `ignoring /initialpose in frame 'camera_init'` 或 `Rejecting initial pose in frame 'camera_init'`，说明 RViz2 Fixed Frame 仍设置错误，应改成 `map` 后重新发送。前者来自 `initialpose_z_fix`，消息会在到达 C++ 定位节点之前被拒绝。
...
|------|-------------|---------------|---------|---------|
| **转弯时整片世界点云随机器人旋转** | RViz2 在 Fixed Frame=`camera_init` 下，`/cloud_registered_1` 中走廊也跟着转 | FAST-LIO 启动日志显示 `flip_yz=false`，或误启了 Python+C++ 双重翻转 | 倒装 MID360 的 IMU 轴向修正未生效或被执行两次 | 确认 `imu_topic: /livox/imu`、`imu_flip_yz: true`，并确保没有启动 `imu_flip.py` |
...
| **转弯稳定后再次移动突然不可恢复漂移** | 停住时短暂稳定，一起步地图/机器人突然跳飞 | `[FAST_LIO_TIMING]` 显示 IMU 数量少或 gap 大；`[FAST_LIO_GUARD]` 连续 rejected | 转弯期间 IMU 丢样/去畸变下降，或绝对有效点门槛误杀长走廊中本来残差正常的稀疏帧 | 检查 QoS/队列/timing；当前以不少于 5 个有效点并联合比例、残差和 correction 判定。单次不安全 IMU propagation 会回滚并重建基线，不再因此永久关闭所有点云；导航 odom/TF 仍在未确认期间停止。 |
...
走廊 profile 的距离裁剪发生在 IMU 去畸变之后、体素降采样之前，不改变原始 scan/IMU 时间同步。日志中的 `[FAST_LIO_RANGE] max=20.0m kept=N/M` 用于确认裁剪确实生效；`N` 长期接近 0 时必须立即停止，不要继续写图。将来增加特征物时，应使用静态、雷达可见的三维箱体/折角/门框等几何，最好有约 `0.5m` 以上的面、交替布置在走廊两侧且往返可见；二维码、AprilTag、反光贴或地面图案不会被当前 FAST-LIO 使用。
...
# 在 workstation 上执行
cd /home/aitech/Workspace/g1_botbrain_greeter
scene=floor1                    # 改为要修改的场景
robot_ip=192.168.37.204         # 改为 G1 实际 IP
maps=botbrain_ws/src/g1_pkg/maps

scp "unitree@${robot_ip}:/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps/${scene}.pgm" \
    "unitree@${robot_ip}:/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps/${scene}.yaml" \
    "$maps/"

# 保存编辑前备份
stamp=$(date +%Y%m%d_%H%M%S)
mkdir -p "$maps/edit_backup_$stamp"
cp -a "$maps/${scene}.pgm" "$maps/${scene}.yaml" \
      "$maps/edit_backup_$stamp/"

# 统一使用可移植的相对路径。map_server 会相对 YAML 所在目录解析它。
sed -i "s|^image:.*|image: ${scene}.pgm|" "$maps/${scene}.yaml"
```
...
cd /home/aitech/Workspace/g1_botbrain_greeter
scene=floor1
bash tools/host_side/map_edit/start_map_edit.sh "/root/maps/${scene}.yaml"
```
...
- `<scene>.yaml` — 配置
- `<scene>.pgm` — 图像（带修改内容）
- `<scene>.json` — VirtualWall 元数据，仅供编辑器再次打开
- `<scene>_region.json` — Region 元数据，仅供编辑器再次打开
...
# 编辑器不写 mode；不存在时补回
grep -q '^mode:' "$maps/${scene}.yaml" || \
  sed -i '/^image:/a mode: trinary' "$maps/${scene}.yaml"

sed -n '1,10p' "$maps/${scene}.yaml"
head -n 4 "$maps/${scene}.pgm"
...
cd /home/aitech/Workspace/g1_botbrain_greeter
scene=floor1
robot_ip=192.168.37.204
maps=botbrain_ws/src/g1_pkg/maps

# Nav2 实际只需要 PGM/YAML
scp "$maps/${scene}.pgm" "$maps/${scene}.yaml" \
    "unitree@${robot_ip}:/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps/"

# JSON 仅在需要保留编辑器元数据时上传
scp "$maps/${scene}.json" \
    "unitree@${robot_ip}:/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps/" \
    2>/dev/null || true
scp "$maps/${scene}_region.json" \
    "unitree@${robot_ip}:/data/unitree/botbrain_ws/botbrain_ws/src/g1_pkg/maps/" \
    2>/dev/null || true
```
...
# 同楼层重载修订后的地图：保留健康的 FAST-LIO，只重建 localization。
# 脚本成功后才重新启动 Navigation。
bash tools/nav/select_map_scene.sh "$scene" \
  --wait-ready \
  --ready-timeout 300 &&
docker compose --profile navigation up -d --force-recreate navigation

# 若脚本提示 FAST-LIO 未运行或不健康，改用：
# bash tools/nav/select_map_scene.sh "$scene" --restart-fast-lio \
#   --wait-ready --ready-timeout 300 && \
#   docker compose --profile navigation up -d --force-recreate navigation

# 确认加载修后的地图和新定位参数；满足检查条件后按 Ctrl-C。
docker compose logs -f localization | \
...
场景选择器返回 `0` 后 Navigation 才会启动。启动后打开 RViz2 导航预设并确认 Fixed Frame=`map`，然后重新执行 **2.3 使用成品地图做 Open3D ICP 验收** 中的 Z/TF、时间戳和连续 accepted 检查。若修订地图的验收不通过，立即停止 Navigation；只有自动全局初始化持续失败时才使用 `/initialpose` 应急回退。
...
每次建新楼层按“一、步骤 1”设置新 `scene`，再通过 Compose 重建 `fast_lio` 服务；`FAST_LIO_MAP_FILE` 会把 PCD 目标显式设为 `<scene>_scans.pcd`，不再手改 `map_file_path`。原 `start_mapping_scene.sh` 仍可作为自动备份/自动等待的可选入口，但不是默认现场流程。

> **每个楼层需要独立建图。** G1 没有多层激光 SLAM，不能自动识别楼层切换。到新楼层后必须选择对应场景；只有该楼层还没有地图时才需要重新建图。

### 4.2 切换地图（导航时用哪张）

标准切图接口只有一个：`bash tools/nav/select_map_scene.sh <场景名> ...`。脚本内部通过 `MAP_SCENE` 创建目标 localization 容器，例如参数 `ug` 自动选择 `ug_scans.pcd + ug.yaml + ug.pgm`，参数 `floor4` 自动选择 `floor4_scans.pcd + floor4.yaml + floor4.pgm`。日常操作不直接设置 `MAP_SCENE`，也不需要软链接、修改 launch 或重新 build。

同一楼层只重载修订后的 PGM/YAML/PCD，且 FAST-LIO 当前健康时，不重建 FAST-LIO：
...
冷启动、机器人重启后首次定位，以及真正跨楼层都必须使用下面带 `--restart-fast-lio` 的流程。脚本会为当前楼层重新建立 `camera_init` 高度基准。
...
# 一条命令会停止旧导航/定位，重建 FAST-LIO 高度基准，等待 IMU，
# 加载并验证目标场景；只有脚本返回 0 才启动新的 Navigation。
bash tools/nav/select_map_scene.sh "$scene" \
  --restart-fast-lio \
  --wait-ready \
  --ready-timeout 300 &&
...
# 必须看到 FAST-LIO timing 连续稳定、实时输出可用；脚本只提供 RViz2 所需的 Zenoh ROS 图。
# FAST-LIO is healthy: IMU initialized, timing stable, outputs live
# Scene '<scene>' loaded. Connect workstation RViz2 through Zenoh.
...
# confidence 和 TF 全部通过。下面日志跟随满足检查条件后按 Ctrl-C。
docker compose logs -f localization | \
...
localization 未指定内部环境键 `MAP_SCENE` 时默认使用 `ug`。需要切换场景时始终使用场景选择器：它会显式传入场景，并验证容器环境与实际加载路径，避免明确切图时仍复用旧 UG 容器。若在 `/data/unitree/botbrain_ws/.env` 中设置 `MAP_SCENE=floor4`，它也只影响以后直接由 Compose 创建的容器，不会让已经运行的 localization 自动切图。

`docker compose restart localization` 只重启旧容器，不会切换地图。跨楼层必须遵守“停止 navigation/localization/fast_lio -> 在目标层重建 FAST-LIO -> 等待 IMU/点云稳定 -> 场景选择器重建 localization -> 等待定位验收 -> 重建 navigation”的顺序；上述标准命令已经封装了该顺序。

若 RViz2 中 `/cloud_registered_1`、`/scan`、`/path_1` 同时显示错误，执行：

```bash
docker compose logs --tail 160 fast_lio | \
  grep -E "FAST_LIO_TIMING|FAST_LIO_GUARD|IMU Initial Done"
```

`discarded buffered LiDAR data, preserved future IMU samples` 是自动恢复日志；`output latched unhealthy` 才表示输出已被安全停止，需要取消导航并重新执行带 `--restart-fast-lio` 的切图命令。RViz2 不显示 `bt_navigator_*_rclcpp_node` 参数面板；点云和 TF 同时消失时应优先检查 FAST-LIO 时序与 Zenoh 连接。
...
楼层识别模块目前尚未实现。后续无论输入来自电梯控制器、二维码、视觉/OCR 还是建筑系统，都只负责产出经过稳定确认的 `scene` 名称，不直接操作 ROS 地图话题。上层状态机必须按以下顺序调用：

1. 取消当前导航目标，并确认机器人已停止。
2. 完成乘梯动作，确认已到目标层、门已打开且机器人站稳。
3. 对楼层结果做去抖和连续确认，得到唯一场景名，例如 `floor4`。
4. 在机器人宿主机执行标准跨层命令。
5. 只有命令返回 `0` 才进入目标楼层导航；任何非零结果都保持停车和 Navigation 停止。

```bash
cd /data/unitree/botbrain_ws
scene=floor4  # 由已稳定确认的楼层检测结果赋值
...
脚本使用 `/tmp/botbrain_map_scene_switch.lock` 防止并发切图；已有切图任务时返回 `75`，上层应延时后重新读取稳定楼层结果再决定是否重试。其他非零返回表示地图文件、FAST-LIO 健康、自动定位或导航输入至少一项未通过，禁止绕过检查启动 Navigation。自动流程不发送 `/initialpose`；机器人无需回到建图起点，绝对初始位姿由目标楼层 PCD 的 FPFH/RANSAC 加 ICP 自动求解。
...
# 查看定位过程；满足检查条件后按 Ctrl-C，不会停止后台容器。
docker compose logs -f localization | \
...
- `--wait-ready` 超时会返回非 0 并保持 Navigation 停止；自动楼层切换流程不发送人工 `/initialpose`
- 在 ready 之前不发布用于导航的猜测 `map -> odom`，RViz2 暂时不能把点云叠到地图上属于预期保护行为
- 只有全局候选长时间无法到达 `3/3` 时，才确认 RViz2 Fixed Frame=`map` 后用 `2D Pose Estimate` 发送 `/initialpose`，并确认 `Manual relocalization applied`
...
- 新地图首次验收仍应按 2.3 稳定观察约 10 秒 accepted；已经验收过的日常地图由场景选择器和 Navigation 自身的两级 preflight 把关
...
# 场景选择器已执行一次 preflight；Navigation 容器启动后会在 sleep 30s 后
# 再执行一次最多 300s 的 preflight，防止两阶段之间传感器或 TF 再次失效。
...
# Workstation RViz2：预设会通过 /g1_robot/goal_pose 发送导航目标
cd ~/Workspace/g1_botbrain_greeter
bash tools/host_side/g1_nav_loc_rviz2.sh 192.168.37.204
```

RViz2 中 Fixed Frame 必须为 `map`。定位已 ready 后，选择 `2D Goal Pose` 并在地图上点击拖拽目标朝向；如需人工重定位，使用 `2D Pose Estimate`，不能在 `camera_init` 下发送。
...
| `fast_lio` | 固定 sleep=`0`；立即启动并等待真实 LiDAR/IMU | `IMU Initial Done`、最近 5 次 timing 至少 `4/5 ok=true` 且最新为 true，并实际读取 `/Odometry_loc` 与 `/cloud_registered_1` |
| `localization` | 固定 sleep=`0`；先发布 `/pcd_map`，再准备 FPFH 并匹配 | `Global localization initialization succeeded`、`/localization_ready=true`，随后连续 3 次 ICP 通过 fitness/RMSE 门限 |
| `navigation` | 固定 sleep=`0`；立即 preflight，独立门禁最多 60s | `Navigation preflight passed`，随后 Nav2 lifecycle 全部 active |
...
此外，`bringup` 不得持续出现 `Move failed: code=3104` 或
`get_balance_mode rc=7301`。机器人当前 Unitree SDK 头文件将它们定义为
`Call api timeout error` 与 `LocoState not available`：前者表示 locomotion API
请求超时，后者表示本体没有可用 LocoState。网卡 UP、路由正确且 ping 通
`192.168.123.161` 只能证明三层网络可达，不能证明本体 locomotion 服务可用；
这两个错误存在时禁止发送导航目标，并按机器人本体服务/运动模式故障处理。
...
local costmap 的配置是 `g1_robot/odom` 坐标系下 `8m x 8m` 的 `rolling_window=true`，正常时机器人应在窗口中心。导航预设已经包含默认关闭的 `Local Costmap (optional)` 图层；需要检查时直接勾选，不需要手动 Add。不能在图层未启用时判断未居中。打开该图层后，再用同一 costmap 时间戳的 TF 做数值验证：
...
## 历史修复记录（仅用于追溯，不作为当前操作手册）

> 以下第三、第四、第五轮内容记录了 2026-07-12 当时的排查演进，部分阈值、初始化确认次数和 timing 恢复行为已被后续实现替换。现场启动、建图、切层和验收一律以前文第一至第五章、当前源码配置以及 `FAST_LIO建图转弯点云漂移问题解决报告.md`、`FAST_LIO转弯后不可恢复漂移第二阶段分析与解决报告.md` 的最新结论为准。不要从历史段落复制参数覆盖当前配置。

### 历史第三轮：转弯后再移动漂移修正（2026-07-12）

本轮修正针对的不是 RViz2 显示问题，而是“旋转后短暂稳定，再次移动时状态或 map→odom 突然跳飞”的恢复链路。
...
当时的局部初始化最低 fitness 被夹紧为不低于 `threshold_fitness_init`，两项均为 0.50：

- `threshold_fitness_init: 0.50`
- `min_initialization_fitness: 0.50`

只有同时通过 fitness、RMSE、单次平移/旋转门限的 candidate 才能更新 map→odom，并且当时的局部初始化成功需要两个不同点云 generation 连续通过。当前标准自动全局初始化以前文为准：最终 `fitness>=0.65`、`rmse<=0.30`，并需要三个一致候选。低质量结果只记录拒绝，不再一轮轮修改初始位姿。
...
推荐验收路线：静止等待 `IMU Initial Done` → 慢速左右转 → 90° 转弯后停稳 → 再前进 1 m → 逐步提高转弯速度。出现跳变时保存完整 FAST-LIO timing/guard 和 Open3D ICP 日志，不要只截 RViz2 图片。

### 历史第四轮：转弯后再移动漂移修正（2026-07-12）
...
- IMU 数量不少于 5；
- 最大 IMU gap 不超过 `common.max_imu_gap`；MID360 当前为 0.06 秒（包含前后 LiDAR 帧边界）；
- scan 时长 0.05~0.15 秒；
...
### rejected/recovery 时 RViz2 的预期表现

- rejected 或 state-only recovery 帧不再发布 `/cloud_registered_1`；
- 因此 RViz2 世界点云会短暂停在上一份可信帧，而不是跟随错误预测旋转；

...
>
> 地图目录：`/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps/`。以下以 `floor1` 为例。
...
`/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/fast_lio/config/mid360.yaml`
...
```bash
maps=/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps
...
导航前先确认 `mid360.yaml` 已设为 `pcd_save_en: false` 并完成编译。Compose 默认加载 `scans.pcd` 和 `accumulated.yaml`，推荐用同目录符号链接成对切换活动地图：

```bash
cd /data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps
...
# 在 workstation 上执行
cd /home/aitech/Workspace/botbrain_project
scp unitree@<G1_IP>:/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps/floor1.{pgm,yaml} \
...
scp botbrain_ws/src/g1_pkg/maps/floor1.{pgm,yaml} \
    "unitree@<G1_IP>":/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps/
...
scp botbrain_ws/src/g1_pkg/maps/floor1_region.json \
    "unitree@<G1_IP>":/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/g1_pkg/maps/ 2>/dev/null || true
```
...
```bash
cd /data/botbrain_ws/botbrain_project-main
cd botbrain_ws/src/g1_pkg/maps

# 确保修后的 2D 图和同一次建图的 PCD 同时成为活动地图
ln -sfn floor1_scans.pcd scans.pcd
ln -sfn floor1.yaml accumulated.yaml
readlink -f scans.pcd
readlink -f accumulated.yaml
...
每个楼层必须独立建图、独立保存 PCD/PGM/YAML，并分别只做一次 PCD 高度校正。例如：

```text
# 1F
floor1_scans_raw.pcd  # 未校正备份
floor1_scans.pcd      # 已校正成品
floor1.pgm
floor1.yaml

# 2F
floor2_scans_raw.pcd
floor2_scans.pcd
floor2.pgm
floor2.yaml
```
...
Compose 默认加载地图目录中的 `scans.pcd` 与 `accumulated.yaml`。推荐通过两个符号链接成对选择同一场景的 3D/2D 地图，不需要修改 launch，也不需要重新 build：
...
# 确认 YAML 内仍指向 floor1.pgm
sed -n '1,10p' floor1.yaml
...
`localization_3d.launch.py` 也支持显式的成对参数。该命令必须在包含 ROS/Open3D 环境的 localization 容器中运行；机器人宿主机不要直接使用 `/botbrain_ws` 路径：

```bash
cd /data/botbrain_ws/botbrain_project-main
docker compose stop localization
...
> 点位文件：`/data/botbrain_ws/botbrain_project-main/botbrain_ws/src/bot_navigation/nav_waypoints.yaml`
>

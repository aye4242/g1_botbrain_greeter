...
```bash
project_dir=/home/aitech/Workspace/g1_botbrain_greeter
...
- Fixed Frame 必须为 `map`（预设默认值）；预设显示 `/map_2d`、`/pcd_map`、`/scan`、`/plan`。
- 只在 `Localization ready` 且 Nav2 lifecycle 就绪后，用工具栏 `Nav2 Goal`（或 `2D Goal Pose`）在地图上点击拖拽，向 `/goal_pose` 发布导航目标。
...
- Fixed Frame：`map`。
- 使用 `configs/g1_nav_loc_rviz2.rviz`；只用 `/pcd_map`、`/cloud_registered_1` 和 `/map_2d` 检查对齐，点云 Decay 设为 `0`。
...
# Workstation RViz2：标准可视化并通过 /goal_pose 发送导航目标
project_dir=/home/aitech/Workspace/g1_botbrain_greeter
g1_ip=192.168.100.30
source /opt/ros/humble/setup.bash
export RMW_IMPLEMENTATION=rmw_zenoh_cpp
...
RViz2 中 Fixed Frame 必须为 `map`。定位已 ready 后，选择 `Nav2 Goal`（或 `2D Goal Pose`）并在地图上点击拖拽目标朝向；如需人工重定位，使用 `2D Pose Estimate`，不能在 `camera_init` 下发送。
